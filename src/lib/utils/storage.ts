...
export const FUREZU_LOCAL_STORAGE = 'FUREZU_LOCAL_STORAGE';
...
export function getLocalStore(storageType: StorageType): StorageValue {
  ...
  const storeRaw =
    storageType === 'local'
      ? localStorage.getItem(FUREZU_LOCAL_STORAGE)
      : sessionStorage.getItem(FUREZU_LOCAL_STORAGE);
...
export function setLocalStore(storageType: StorageType, data: StorageValue): StorageValue {
  ...
  if (storageType === 'local') {
    localStorage.setItem(FUREZU_LOCAL_STORAGE, storeData);
  } else {
    sessionStorage.setItem(FUREZU_LOCAL_STORAGE, storeData);
  }
